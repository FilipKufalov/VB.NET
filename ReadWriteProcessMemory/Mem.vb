...
Public Class Mem
    ...
    Public Shared m_BaseAddress As IntPtr
    ...
    <DllImport("kernel32.dll")>
    Public Shared Function ReadProcessMemory(ByVal hProcess As IntPtr, ByVal lpBaseAddress As IntPtr, ByVal buffer As Byte(), ByVal size As Integer, ByRef lpNumberOfBytesRead As Integer) As Boolean
    End Function

    <DllImport("kernel32.dll")>
    Public Shared Function WriteProcessMemory(ByVal hProcess As IntPtr, ByVal lpBaseAddress As IntPtr, ByVal buffer As Byte(), ByVal size As Integer, ByRef lpNumberOfBytesWritten As Integer) As Boolean
    End Function
    ...
    Public Shared Function Initialize(ByVal ProcessName As String, ByRef windowsTitle As String) As Boolean
        If (Process.GetProcessesByName(ProcessName).Length > 0) Then
            ...
            m_BaseAddress = Process.GetProcessesByName(ProcessName)(0).MainModule.BaseAddress
    ...
    Public Shared Function Inject(ByVal dllPath As String) As Boolean
        If m_pProcessHandle = IntPtr.Zero Then
            Return False
        End If
        ...
        If allocAddress = IntPtr.Zero Then
            Return False
        End If

        Dim kernelMod As IntPtr = GetModuleHandle("kernel32.dll")
        Dim loadLibAddr = GetProcAddress(kernelMod, "LoadLibraryA")

        If kernelMod = IntPtr.Zero Or loadLibAddr = IntPtr.Zero Then
            Return False
        End If

        WriteProcessMemory(m_pProcessHandle, allocAddress, dllBytes, dllBytes.Length, 0)
        Dim libThread As IntPtr = CreateRemoteThread(m_pProcessHandle, 0, 0, loadLibAddr, allocAddress, 0, 0)

        If libThread = IntPtr.Zero Then
            Return False
        Else
    ...
    Public Shared Function GetEntryAddr(ByVal baseEntry As Integer, ByVal entryId As Integer) As IntPtr
        Return ReadMemory(Of IntPtr)(baseEntry + (&HC * entryId) + &H8)
    End Function
    ...
    Public Shared Function WriteMemory(Of T As Structure)(ByVal Adress As Integer, ByVal write As Object) As T
        ...
        WriteProcessMemory(m_pProcessHandle, Adress, buffer, buffer.Length, m_iNumberOfBytesRead)
        Return ByteArrayToStructure(Of T)(buffer)
    End Function
    ...
    Public Shared Function WriteFloatMem(ByVal Adress As Integer, ByVal write As Single)
        WriteFloatMemory(m_pProcessHandle, Adress, write, 4, Nothing)
    End Function
    ...
    Public Declare Function WriteByteMemory Lib "kernel32" Alias "WriteProcessMemory" (ByVal hProcess As IntPtr, ByVal lpBaseAddress As IntPtr, ByRef lpBuffer As Byte, ByVal nSize As Integer, ByRef lpNumberOfBytesWritten As Integer) As Integer

    Public Shared Function WriteByteMem(ByVal Adress As IntPtr, ByVal write As Byte)
        WriteByteMemory(m_pProcessHandle, Adress, write, 1, Nothing)
    End Function

    Private Declare Function WriteIntMemory Lib "kernel32" Alias "WriteProcessMemory" (ByVal hProcess As IntPtr, ByVal lpBaseAddress As IntPtr, ByRef lpBuffer As Integer, ByVal nSize As Integer, ByRef lpNumberOfBytesWritten As Integer) As Integer

    Public Shared Sub WriteIntMem(ByVal Adress As IntPtr, ByVal write As Integer)
        WriteIntMemory(m_pProcessHandle, Adress, write, 4, 0)
    End Sub
